...
public class employeeTransaction extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					employeeTransaction frame = new employeeTransaction();
	...
	public employeeTransaction() {
		...
		scrollPane.setBounds(568, 209, 478, 450);
        ...
        JLabel lblNewLabel = new JLabel("Fuel Supply Transaction Table");
        ...
        JLabel FirstNamelbl = new JLabel("Supplier ID\r\n");
        ...
        JLabel emailLbl = new JLabel("Liter Supplied");
        ...
        empIDtext.setBounds(183, 10, 237, 28);
        ...
        insertButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                int fuelSupplierID=Integer.parseInt(empIDtext.getText());
                ...
                try {
                    ...
                    PreparedStatement checkStatement = connection.prepareStatement("SELECT FuelSupplierID FROM fuelsuppliers WHERE FuelSupplierID = ?");
                    checkStatement.setInt(1, fuelSupplierID);
                    ResultSet checkResult = checkStatement.executeQuery();

                    if (!checkResult.next()) {

                        // Customer ID does not exist, show message
                    	JOptionPane.showMessageDialog(insertButton, "Fuel Supplier ID does not exist.");
                        
                    } else {
                        // Customer ID exists, insert data into the transactions table
                    	PreparedStatement st = connection.prepareStatement("INSERT INTO fuelsuppliertransactions (FuelSupplierID, LitersSupplied, TotalPrice, PaymentMethod, TransactionDate) VALUES (?, ?, ?, ?, ?)");
                        
                        st.setInt(1, fuelSupplierID);
                        st.setInt(2, liter);
                        st.setInt(3, price);
        ...
        updatebutton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                try {
                    int cusID = Integer.parseInt(empIDtext.getText());
                    int liter = Integer.parseInt(litertextField.getText());
                    int price = Integer.parseInt(pricetextField.getText());
                    String paymentMethod = (String) paymentcomboBox.getSelectedItem();
                    Date date = calendar.getDate();

                    int supplierTransactionID = Integer.parseInt(table.getValueAt(table.getSelectedRow(), 0).toString());

                    // Check if the FuelSupplierID value exists in the fuelsuppliers table
                    PreparedStatement st = connection.prepareStatement("SELECT FuelSupplierID FROM fuelsuppliers WHERE FuelSupplierID = ?");
                    st.setInt(1, cusID);
                    ResultSet rs = st.executeQuery();

                    if (!rs.next()) {
                        // FuelSupplierID does not exist, display an error message
                        JOptionPane.showMessageDialog(updatebutton, "FuelSupplierID does not exist in the fuelsuppliers table.");
                    } else {
                        // FuelSupplierID exists, proceed with the update
                        PreparedStatement st1 = connection.prepareStatement("UPDATE fuelsuppliertransactions SET LitersSupplied = ?, TotalPrice = ?, PaymentMethod = ?, FuelSupplierID = ? WHERE FuelSupplierTransactionID = ?");
                        st1.setInt(1, liter);
                        st1.setInt(2, price);
                        st1.setString(3, paymentMethod);
                        st1.setInt(4, cusID);
                        st1.setInt(5, supplierTransactionID);
                        st1.executeUpdate();
        ...
        DeleteButton.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		try {
					...
					String deletefuelsupplySQL = "DELETE FROM fuelsuppliers WHERE customer_id = " +CustransactionID ;
		            st.executeUpdate(deletefuelsupplySQL);
		...
		JButton searchButton = new JButton("Search");
	     ...
	     searchButton.addActionListener(new ActionListener() {
	            public void actionPerformed(ActionEvent e) {
	                ...
	                try {
	                    PreparedStatement st3 = connection.prepareStatement("SELECT * FROM fuelsuppliertransactions WHERE FuelSupplierID LIKE ? or FuelSupplierTransactionID LIKE ? ");
	                    for (int i = 1; i <= 2; i++) {
	...
	private void refreshTable() {
        ...
        try {
            PreparedStatement st3 = connection.prepareStatement("Select * From fuelsuppliertransactions");
